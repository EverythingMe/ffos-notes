...
(function(window) {
  ...
  var localeChangeEvent = new CustomEvent(
    "localechange",
    {
      detail: {
  ...
  document.addEventListener('visibilitychange', function() {
      if (document.hidden) return; // user went off page, ignore
      var locale = navigator.language.substr(0, 2);
      if (navigator.mozL10n.currentLocale == locale) return; // no change
      navigator.mozL10n.currentLocale = locale;
      document.dispatchEvent(localeChangeEvent);
  }, false);
  document.addEventListener('localechange', function(){
    loadLocale(navigator.mozL10n.currentLocale, translateFragment);
  }, false);
