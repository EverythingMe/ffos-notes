...
class t_js_generator : public t_oop_generator {
 ...
 private:
  ...
  /**
   * True iff we should generate callback-style functions
   */
  bool gen_async_;
...
void t_js_generator::generate_service_client(t_service* tservice) {
  ...
  for (f_iter = functions.begin(); f_iter != functions.end(); ++f_iter) {
    ...
    f_service_ <<  js_namespace(tservice->get_program())<<service_name_<<"Client.prototype." <<
      function_signature(*f_iter, "", gen_node_ || gen_jquery_ || gen_async_) << " {" << endl;
    ...
    if (gen_jquery_) {
      ...
      std::string callback = "function() { callback.onSuccess(me.recv_" + funname + "()); }";
      f_service_ << "var me = this;" << endl << "try {" << endl
       << indent() << "this.send_" << funname << "(" << arglist 
       << (arglist.empty() ? "" : ", ") << callback << ");" << endl
       << "} catch(e) {" << endl
        << indent() << indent() << "callback.onFailure(e);" << endl
        << indent() << "}" << endl;
      indent_down();
    }
    indent_down();
    ...
    f_service_ <<  js_namespace(tservice->get_program())<<service_name_ <<
        "Client.prototype.send_" << function_signature(*f_iter, "", gen_jquery_ || gen_async_) << " {" <<endl;
